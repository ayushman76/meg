import service from '../export/treatment.js';
...
var my_arr = [arr,arr01,arr02,arr03,t_arr0 ,t_arr01 ,t_arr02,t_arr03,s_arr,s_arr01,s_arr02,s_arr03];

document.querySelector('.treatment').innerHTML +='<div class="container head"><div class="row"><h1 class="text-center">Service (Laparoscopic Surgery)</h1></div></div>';

for(var i in my_arr){
	document.querySelector('.treatment>.container>.row').innerHTML +=`
	   <div class="col-md-3">
	       <div class="myService wow fadeIn">
		       <img src="${my_arr[i].img}" alt="${my_arr[i].img}" class="img-thumbnail">
			   <p class="head01">${my_arr[i].details}</p>
		   </div>
	   </div>
	`;
}
